...
def _setup_docs(tmp_path: Path) -> Path:
    """Copy fixture docs to tmp_path/devlead_docs."""
...
def test_rollover_creates_archive_dir(tmp_path):
    """Rollover creates devlead_docs/archive/."""
    docs = _setup_docs(tmp_path)
    files = ["_project_tasks.md", "_intake_issues.md"]
...
def test_rollover_creates_archive_files(tmp_path):
    ...
    assert (docs / "archive" / "_intake_issues_2026-04.md").exists()
...
def test_rollover_carries_forward_open(tmp_path):
    ...
    docs = _setup_docs(tmp_path)
    do_rollover(docs, ["_intake_issues.md"], today=date(2026, 4, 5))
    current = (docs / "_intake_issues.md").read_text()
    # OPEN items remain
    assert "ISS-001" in current
    assert "ISS-002" in current
    assert "ISS-004" in current
    # CLOSED items removed
    assert "ISS-003" not in current
...
def test_rollover_preserves_header(tmp_path):
    ...
    do_rollover(docs, ["_intake_issues.md"], today=date(2026, 4, 5))
    current = (docs / "_intake_issues.md").read_text()
    assert "# Issue Intake" in current
...
def test_rollover_idempotent_same_month(tmp_path):
    ...
    docs = _setup_docs(tmp_path)
    files = ["_intake_issues.md"]
    do_rollover(docs, files, today=date(2026, 4, 5))
    first_archive = (docs / "archive" / "_intake_issues_2026-04.md").read_text()
    do_rollover(docs, files, today=date(2026, 4, 5))
    second_archive = (docs / "archive" / "_intake_issues_2026-04.md").read_text()
