...
def tmp_project(tmp_path):
    """Create a temporary project directory with devlead_docs/."""
...
.venv/
.pytest_cache/
devlead_docs/session_state.json
```
...
Add `start`, `gate`, `transition`, `checklist` commands to cli.py dispatch. Resolve `state_file` from cwd: `Path.cwd() / "devlead_docs" / "session_state.json"`.

- [ ] **Step 6: Manual integration test**

```bash
mkdir -p /tmp/test_devlead/devlead_docs && cd /tmp/test_devlead
...
Test: init creates devlead_docs/ with all files, creates devlead.toml, merges hooks into .claude/settings.json (preserving existing), adds gitignore entry, idempotent skip on second run.
...
Checks: devlead_docs/ exists, devlead.toml found, .claude/settings.json has hooks, session_state.json in gitignore, all expected markdown files exist, tables have expected columns.
...
On transition to SESSION_END: compute session metrics from state (circles from transition count, gate violations, etc.), append JSON line to `devlead_docs/session_history.jsonl`.
