...
Upgrades replace only the engine code. `devlead_docs/` files in the target project are never touched by upgrades — the user owns those files after `devlead init`.
...
This removes the CLI. The `devlead_docs/` folder and all project docs remain untouched — they are plain markdown, readable without DevLead. The hooks in `.claude/settings.json` will show errors (command not found) and should be manually removed.
...
| UPDATE | `memory_derived` | `MEMORY.md` regenerated from `devlead_docs/` |
...
```json
// devlead_docs/session_state.json (gitignored)
{
    "state": "ORIENT",
...
| Living | `_living_*.md` | `devlead_docs/` | Evergreen reference — architecture, standards, guides |
| Intake | `_intake_*.md` | `devlead_docs/` | Incoming requests — bugs, features, gaps, changes |
| Project | `_project_*.md` | `devlead_docs/` | Work plan — status, tasks, roadmap |
...
`~/.claude/memory/MEMORY.md` is **derived from** `devlead_docs/`. It stays at Claude's conventional location but contains only an index pointing to `devlead_docs/` files. Never the reverse.
...
Requires `devlead_docs/session_history.jsonl` — one line appended per completed session:
```jsonl
...
The doc parser reads `devlead_docs/` files and produces these variables:
...
- Receives built-in variables + path to `devlead_docs/`
...
```bash
devlead init                     # Scaffold devlead_docs/, configure hooks
...
1. Creates `devlead_docs/` with template files from `scaffold/`
2. Creates `devlead.toml` with defaults
3. Merges hook config into `.claude/settings.json` (preserves existing hooks)
4. Adds `devlead_docs/session_state.json` to `.gitignore`
...
[project]
name = "my-project"
docs_dir = "devlead_docs"           # Override if needed
...
├── ~/projects/eTrading/devlead_docs/     # Project A
├── ~/projects/income_desk/devlead_docs/  # Project B
└── ~/projects/devlead/devlead_docs/      # Project C
```
...
- **Does not lock you in.** If you uninstall, your `devlead_docs/` files still work as plain markdown.
