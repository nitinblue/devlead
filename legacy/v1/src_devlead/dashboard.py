...
def generate_dashboard_html(
    ...
    # Load file metadata for all devlead_docs files
    ...
    tab_business = _tab_business(epics, stories, tasks, vars, kpis, status_text, objectives, objectives_text, docs_dir=docs_dir)
...
def _files_section(file_metas: list[dict]) -> str:
    """Show user-editable devlead_docs files with metadata."""
    if not file_metas:
        return ""
...
def _tab_business(epics: list[dict], stories: list[dict], tasks: list[dict], vars: dict, kpis: list, status_text: str, objectives: list[dict] = None, objectives_text: str = "", docs_dir: Path | None = None) -> str:
    ...
    # --- Weighted Phase Convergence hero ---
    _wb_bos = None
    try:
        if docs_dir:
            from devlead.workbook import load_workbook as _load_wb
            from devlead.convergence import (
                phase_convergence as _phase_conv,
                bo_convergence as _bo_conv,
                tbo_convergence as _tbo_conv,
            )
            _wb = _load_wb(docs_dir)
            if _wb.bos:
                _wb_bos = _wb.bos
    except Exception:
        _wb_bos = None
    ...
    if _wb_bos:
        phase_pct = int(_phase_conv(_wb_bos))
        phase_label = f"Phase Convergence — {_wb_bos[0].phase}" if _wb_bos[0].phase else "Phase Convergence (weighted)"
    else:
        tbo_total = len(objectives) if objectives else 0
        tbo_done = sum(1 for o in (objectives or []) if "DONE" in o.get("Status", "").upper())
        phase_pct = int(tbo_done / tbo_total * 100) if tbo_total > 0 else 0
        phase_label = "TBO Convergence"

    tbo_total_display = len(objectives) if objectives else 0
    tbo_done_display = sum(1 for o in (objectives or []) if "DONE" in o.get("Status", "").upper())

    html += f"""
    <div class="section">
        <div class="convergence-hero">
            <div class="conv-number" style="color: {_pct_color(phase_pct)}">{phase_pct}%</div>
            <div class="conv-label">{phase_label}</div>
            <div style="width:80%;max-width:400px;height:12px;background:#1e293b;border-radius:6px;margin:0.5rem auto 0;overflow:hidden;">
                <div style="height:100%;width:{phase_pct}%;background:{_pct_color(phase_pct)};border-radius:6px;transition:width 0.3s;"></div>
            </div>
            <div class="conv-label" style="margin-top:0.3rem;font-size:0.85rem;">{tbo_done_display} of {tbo_total_display} business outcomes delivered</div>
        </div>
    </div>
    """

    # --- Per-BO Convergence Cards ---
    if _wb_bos:
        bo_cards = ""
        for bo in _wb_bos:
            bo_pct = int(_bo_conv(bo))
            bo_color = _pct_color(bo_pct)
            tbo_rows_html = ""
            for tbo in bo.tbos:
                t_pct = int(_tbo_conv(tbo))
                t_color = _pct_color(t_pct)
                tbo_rows_html += f"""
                <div style="display:flex;align-items:center;gap:0.5rem;padding:0.3rem 0;border-bottom:1px solid #1e293b;">
                    <span class="mono" style="width:60px;flex-shrink:0;color:var(--accent);">{_esc(tbo.id)}</span>
                    <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{_esc(tbo.objective[:50])}</span>
                    <span class="muted" style="width:40px;text-align:right;">wt:{tbo.weight}</span>
                    <span style="width:45px;text-align:right;font-weight:600;color:{t_color};">{t_pct}%</span>
                    <div style="width:80px;height:8px;background:#1e293b;border-radius:4px;overflow:hidden;">
                        <div style="height:100%;width:{t_pct}%;background:{t_color};border-radius:4px;"></div>
                    </div>
                </div>"""
            bo_cards += f"""
            <div style="background:#0f172a;border:1px solid #334155;border-radius:8px;padding:1rem;margin-bottom:1rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.75rem;">
                    <div>
                        <span class="mono" style="color:var(--accent);font-weight:700;">{_esc(bo.id)}</span>
                        <span style="margin-left:0.5rem;">{_esc(bo.objective[:50])}</span>
                        <span class="muted" style="margin-left:0.5rem;">wt:{bo.weight}</span>
                    </div>
                    <span style="font-size:1.3rem;font-weight:700;color:{bo_color};">{bo_pct}%</span>
                </div>
                <div style="height:8px;background:#1e293b;border-radius:4px;overflow:hidden;margin-bottom:0.75rem;">
                    <div style="height:100%;width:{bo_pct}%;background:{bo_color};border-radius:4px;"></div>
                </div>
                {tbo_rows_html}
            </div>"""
        html += f"""
        <div class="section">
            <h2>Business Objective Convergence</h2>
            {bo_cards}
        </div>
        """
...
def _tab_audit(entries: list[dict]) -> str:
    ...
    Filters out .py source file edits. Shows: devlead_docs changes,
    ...
    for e in entries:
        ...
        # Always show: cross-project, memory writes, devlead_docs edits, config edits
        if cross or "/devlead_docs/" in norm or "/.claude/" in norm or "/memory/" in norm or "devlead.toml" in norm:
