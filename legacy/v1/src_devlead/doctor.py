...
def do_doctor(project_dir: Path) -> list[dict]:
    ...
    # 1. devlead_docs/ exists
    if docs_dir.is_dir():
        results.append({"check": "devlead_docs/ exists", "status": "OK", "detail": ""})
    else:
        results.append({
            "check": "devlead_docs/ exists",
            "status": "ERROR",
    ...
    if gitignore.exists() and "session_state.json" in gitignore.read_text():
        ...
        results.append({
            "check": "session_state.json is gitignored",
            "status": "WARN",
            "detail": "Add 'devlead_docs/session_state.json' to .gitignore",
        })
