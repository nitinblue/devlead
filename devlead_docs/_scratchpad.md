...
## Entry - 2026-04-16 - Session 4 key concepts crystallized (Nitin + Claude 2026-04-16)

Three core concepts emerged and were validated this session. Documenting for permanence.

## 1. Memory Management

Problem: Claude starts every session from zero. Files exist but Claude skips them.
Solution: Embed governance content directly in CLAUDE.md (not pointers to files).
CLAUDE.md is the ONLY file every LLM reads automatically at session start.
Everything load-bearing goes IN CLAUDE.md, not referenced from it.

Key insight: _resume.md must be auto-generated by DevLead from real data
(hierarchy convergence, intake status, audit log, git state). Claude never
hand-writes it. DevLead writes it via devlead resume command.

Session history (_session_history.jsonl) tracks convergence + tokens across
sessions for trend analysis.

Status: resume.py built and working. Session history schema defined but
record_session() never called yet (zero entries).

## 2. Documentation as Governance

Problem: Claude builds Python modules and calls it governance. The code exists
but nothing governs Claude's behavior.

Solution: The governance IS the documentation. Specifically:
- _routing_table.md content embedded in CLAUDE.md = the brain
- _project_hierarchy.md with verify: fields on TTOs = the acceptance criteria  
- devlead report HTML = the trust layer for non-coders

MD-driven means: if you delete all the Python, the routing table in CLAUDE.md
still tells the LLM what to do. Python adds computation (KPIs, reports) but
governance works without it.

Status: routing table created with 5 responsibilities. Embedded in CLAUDE.md
via bootstrap.py. Proven mechanism (same as superpowers skills).

## 3. Intent Routing

Problem: Claude improvises. Given any user input, Claude guesses what to do.

Solution: Deterministic routing. User intent is classified against DevLead
responsibilities. If matched, follow predefined steps (specific file reads
and writes). If unmatched, Claude proceeds as normal.

The routing table defines:
- Triggers (what user might say)
- Steps (exact sequence of READ/WRITE/VALIDATE/ASK actions)
- Guards (invariants that must hold)

Key insight from Nitin: responsibilities are configurable. Adding a new one
= adding a markdown section. No code changes. The LLM reads the new section
and follows it.

Feasibility proof: superpowers skills work this exact way. Markdown loaded
into context, LLM follows the steps. The routing table is a permanent skill
that is always active via CLAUDE.md.

Status: 5 responsibilities defined (R1 pipeline, R2 no dark code, R4 deadlines,
R5 project mgmt, R6 KPIs). Embedded in CLAUDE.md. Not yet tested in a fresh
session — that is the critical next step.

## 4. Definition of Done

Problem: Claude marks its own work done. Every checkbox is Claude grading itself.

Solution: TTOs have verify: fields — shell commands that DevLead runs. If exit 0,
the TTO is done. Claude NEVER checks functional TTO boxes. Only DevLead does.

Non-functional TTOs (documentation, pricing, positioning) can be marked by Claude
since they require human judgment.

Status: 45 TTOs with verify: fields. All unchecked. DevLead verification runner
not built yet — that is the mechanism that will actually check boxes.

## Open questions for next session

1. Should the routing table be a skill (invoked via Skill tool) or always-in-CLAUDE.md?
2. How does intent classification actually work? Pattern matching on keywords? Or LLM judgment?
3. What happens when intent is ambiguous — matches two responsibilities?
4. How do we test routing in a fresh session without burning all tokens on the test?
