...
def test_gate_blocks_execute_in_orient(state_file):
    """EXECUTE gate blocks when current state is ORIENT."""
    state = init_state()
    state["state"] = "ORIENT"
    save_state(state, state_file)
    with pytest.raises(SystemExit) as exc:
...
def test_gate_plan_allows_in_intake(state_file):
    """PLAN gate allows in INTAKE (entering plan mode from intake)."""
    state = init_state()
    state["state"] = "INTAKE"
    save_state(state, state_file)
    with pytest.raises(SystemExit) as exc:
...
def test_gate_plan_blocks_in_orient(state_file):
    """PLAN gate blocks in ORIENT (must intake first)."""
    state = init_state()
    state["state"] = "ORIENT"
    save_state(state, state_file)
    with pytest.raises(SystemExit) as exc:
...
def test_gate_missing_state_file(state_file):
    """Gate blocks when state file doesn't exist."""
    with pytest.raises(SystemExit) as exc:
        check_gate(state_file, "EXECUTE")
    assert exc.value.code == 2
...
def test_transition_valid(state_file):
    """Valid transition with complete checklist succeeds."""
    state = init_state()
    state["state"] = "ORIENT"
...
def test_transition_invalid(state_file):
    """Invalid transition (ORIENT -> EXECUTE) is blocked."""
    state = init_state()
    state["state"] = "ORIENT"
    for key in state["checklists"]["ORIENT"]:
        state["checklists"]["ORIENT"][key] = True
...
def test_transition_blocked_by_incomplete_checklist(state_file):
    """Transition blocked when exit criteria not met."""
    state = init_state()
    state["state"] = "ORIENT"
